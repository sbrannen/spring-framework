...
`maximumNestedPathDepth` value is configurable. You can specify a custom value via
`setMaximumNestedPathDepth(...)` on a `ConfigurablePropertyAccessor` such as
...
Note that this limit applies to property binding only. It is not applicable to
<<data-binding-constructor-binding,constructor binding>> via `DataBinder.construct`,
whose nesting depth is instead bounded by the declared constructor parameters of the
target type.
...
The `maximumNestedPathDepth` limit improves diagnostics for that common case by converting
