...
When using an `@Bean` method to register a `BeanPostProcessor`, declare the method
as `static` and ideally with no dependencies. Doing so avoids eager initialization of
the configuration class and other beans, which would make them ineligible for full
post-processing (such as auto-proxying). See the "BeanPostProcessor-returning
`@Bean` methods" section in the {spring-framework-api}/context/annotation/Bean.html[`@Bean`]
javadoc for details.
...
Alternatively, the following `beans` element uses the `InstantiationTracingBeanPostProcessor`
...
When registering a `BeanFactoryPostProcessor` via an `@Bean` method on a
