...
`org.springframework.context.annotation` can be used for such purposes. For naming
conflicts among `@Bean` methods in `@Configuration` classes, use
`FullyQualifiedConfigurationBeanNameGenerator` (as of Spring Framework 7.0), which
generates fully-qualified default bean names (`className.methodName`) for `@Bean`
methods without an explicit `name` attribute; set it via
`AnnotationConfigApplicationContext.setBeanNameGenerator()` prior to registering
