...
public interface CompilableIndexAccessor extends IndexAccessor, Opcodes {
	 ...
	 * <p>May only be known once the index has been accessed.
	 */
	...
	/**
	 * Generate bytecode that performs the index access operation into the
	 * specified {@link MethodVisitor} using context information from the
	 * {@link CodeFlow} where necessary.
	 * <p>The supplied {@code indexNode} should be used to generate the
	 * appropriate code to load the index onto the stack. For example, given the
	 * expression {@code book.authors[0]}, invoking
