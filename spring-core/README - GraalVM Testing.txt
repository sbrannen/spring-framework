...
4. Run the tests using the standard Gradle test task with `-DnativeImageTesting=true` in
...
5. Execute `runTestsWithAgent.sh` to run the tests with the GraalVM JVM agent.

------------------------------------------------------------------------------------------

6. Execute `ni.sh` to compile a `spring-core-tests.bin` native image executable in the
   `build` directory using the configuration generated by the GraalVM JVM agent in the
   `build/graalvm` directory combined with the hand-crafted configuration found in
   `src/test/resources/META-INF/native-image`.

------------------------------------------------------------------------------------------

7. Execute `runTestsInNativeImage.sh` to run the tests in the native image.
