...
6. Run the tests using the standard Gradle test task with `-DnativeImageTesting=true` in
...
7. `getActiveTests.sh` is used to convert `build/test_classes.txt` into a single string
...
8. Execute `runTestsWithAgent.sh` to run the tests with the GraalVM JVM agent.

------------------------------------------------------------------------------------------

9. Execute the `ni.sh` script which compiles a `spring-core-tests.bin` native image
...
10. Execute `runTestsInNativeImage.sh` to run the tests in the native image.
