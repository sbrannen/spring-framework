...
5. `getActiveTests.sh` is used to convert `build/test_classes.txt` into a single string
...
6. Execute `runTestsWithAgent.sh` to run the tests with the GraalVM JVM agent.

------------------------------------------------------------------------------------------

7. Execute the `ni.sh` script which compiles a `spring-core-tests.bin` native image
...
8. Execute `runTestsInNativeImage.sh` to run the tests in the native image.
