...
class CompositeRetryListenerTests {
	...
	void onRetryPolicyTimeout() {
		...
		RetryException exception = new RetryException("", new Exception());
		compositeRetryListener.onRetryPolicyTimeout(retryPolicy, retryable, exception, elapsedTime);

		verify(listener1).onRetryPolicyTimeout(retryPolicy, retryable, exception, elapsedTime);
		verify(listener2).onRetryPolicyTimeout(retryPolicy, retryable, exception, elapsedTime);
		verify(listener3).onRetryPolicyTimeout(retryPolicy, retryable, exception, elapsedTime);
	}
