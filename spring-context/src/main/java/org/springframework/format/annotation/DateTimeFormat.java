 ...
 * be used when formatting date time values. Whenever the {@link #iso} attribute
 * is used, {@code UTC} will be used as the time zone. The same time zone will be
 * applied to any {@linkplain #fallbackPatterns fallback patterns} as well. In
 * order to enforce consistent use of {@code UTC} as the time zone, you can
 * bootstrap the JVM with {@code -Duser.timezone=UTC}.
 *
 * @author Keith Donald
...
public @interface DateTimeFormat {
	 ...
	 *
	 * <pre style="code">
	 * {@literal @}DateTimeFormat(iso = ISO.DATE, fallbackPatterns = { "M/d/yy", "yyyyMMdd", "yyyy.MM.dd" })
	 * </pre>
	 *
