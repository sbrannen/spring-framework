...
class ContextCache {
	 ...
	 * {@linkplain ConfigurableApplicationContext#close() close} it if it is an instance
	 * of {@link ConfigurableApplicationContext}.
	 *
	 * <p>Generally speaking, you would only call this method if you change the state of a
	 * singleton bean, potentially affecting future interaction with the context.
	 *
	 * <p>In addition, if the context with the supplied key serves as a parent to any other
	 * contexts currently stored in the cache, those child contexts will be
	 * <em>removed</em> as well, and this algorithm will be applied recursively to lower
	 * levels in the context hierarchy.
	 *
	 * @param key the context key (never {@code null})
	 */
	void remove(MergedContextConfiguration key) {
		...
		synchronized (monitor) {
			...
			remove(removedContexts, key);

			// Remove all remaining references to any removed contexts from the hierarchy
			// map.
	...
	private void remove(List<MergedContextConfiguration> removedContexts, MergedContextConfiguration key) {
		...
		synchronized (monitor) {
			...
			if (children != null) {
				...
				// Remove the set of children for the current context from the hierarchy
				// map.
	...
	/**
	 * Generates a text string, which contains the {@link #size() size} as well as the
	 * {@link #getHitCount() hit} and {@link #getMissCount() miss} counts.
	 */
