...
public class TestContextAotGenerator {
	...
	public void processAheadOfTime(Stream<Class<?>> testClasses) throws TestContextAotException {
		...
		try {
			...
			Set<RuntimeHintsRegistrar> runtimeHintsRegistrars = new LinkedHashSet<>();
			...
			testClasses.forEach(testClass -> {
				...
				MergedAnnotations.from(testClass, SearchStrategy.TYPE_HIERARCHY).stream(ImportRuntimeHints.class)
						.map(MergedAnnotation::synthesize)
						.map(ImportRuntimeHints::value)
						.flatMap(Arrays::stream)
						.map(BeanUtils::instantiateClass)
						.forEach(runtimeHintsRegistrars::add);
			...
			runtimeHintsRegistrars.forEach(registrar -> {
