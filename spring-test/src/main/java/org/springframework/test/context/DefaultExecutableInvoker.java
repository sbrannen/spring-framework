 ...
 *
 * <p>This implementation never provides arguments to an
 * {@link java.lang.reflect.Executable Executable}.
 *
 * @author Sam Brannen
 * @since 6.1
 */
class DefaultExecutableInvoker implements ExecutableInvoker {
	...
	public <T> T invoke(Constructor<T> constructor) throws Exception {
		...
		try {
			...
			if (logger.isErrorEnabled()) {
				logger.error("Exception encountered while invoking constructor [%s]"
						.formatted(constructor), ex.getTargetException());
			}
