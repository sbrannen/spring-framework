...
class TestNGConcurrencyTests {
	...
	void runTestsInParallel() throws Exception {
		...
		assertThat(listener.testStartCount).as("tests started").isEqualTo(10);
		assertThat(listener.testSuccessCount).as("successful tests").isEqualTo(10);
		assertThat(listener.testFailureCount).as("failed tests").isEqualTo(0);
		assertThat(listener.failedConfigurationsCount).as("failed configurations").isEqualTo(0);
