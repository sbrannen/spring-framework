 ...
 * when the manager is asked to update the mutable state of the context
 * from concurrently executing threads.
 *
 * <p>Ideally, mutated state should only be visible to the thread in
...
public class TestContextConcurrencyTests {
		...
		IntStream.range(1, 20).forEach(i -> {
			...
			stream(TestCase.class.getDeclaredMethods()).parallel().forEach(testMethod -> {
				try {
					...
					// we don't actually need to invoke the test methods.
	...
	private static class TrackingListener implements TestExecutionListener {
		...
		public void beforeTestMethod(TestContext testContext) throws Exception {
			actualMethods.add(testContext.getTestMethod().getName());
		}
